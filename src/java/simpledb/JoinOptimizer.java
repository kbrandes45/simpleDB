...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        if (joinOp == Predicate.Op.EQUALS) { //should this include like or notequals?
        	if (!t1pkey && !t2pkey) {
        		//neither attribute is primary key
        		if (card1 > card2) 
        			return card1;
        		else
        			return card2;
        	...
        	if (t1pkey) {
        		//table 1 has primary key, so table 2 is limit
        		return card2;
        	}
        	if (t2pkey)
        		return card1;
        	...
        	//Range scans
        	int cart_pdt = card1*card2;
        	int ret_card = (int) (cart_pdt*.75);
        	if (ret_card < card2)
        		ret_card = card2;
        	if (ret_card < card1) 
				ret_card = card1;
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	int num_joins = this.joins.size();
    	PlanCache pc = new PlanCache();
    	Set<Set<LogicalJoinNode>> size_1_subs = enumerateSubsets(this.joins, 1);
    	
    	for (int i = 0; i<=num_joins; i++) {
    		for (Set<LogicalJoinNode>  subset : enumerateSubsets(this.joins, i)){
    			CostCard best_cc = new CostCard();
    			best_cc.plan = null;
    			best_cc.cost = Double.MAX_VALUE;
    			best_cc.card = Integer.MAX_VALUE;
    			for (LogicalJoinNode s : subset) {
    				CostCard current_cc = computeCostAndCardOfSubplan(stats, filterSelectivities, 
    						s, subset, best_cc.cost, pc);
    				if (current_cc != null && current_cc.cost < best_cc.cost)
    					best_cc = current_cc;
    			}
    			pc.addPlan(subset, best_cc.cost, best_cc.card, best_cc.plan);			
    		}
    	}
    	Set<LogicalJoinNode> starting_joins = new HashSet<LogicalJoinNode>(this.joins);
    	Vector<LogicalJoinNode> best_ordering = pc.getOrder(starting_joins);
